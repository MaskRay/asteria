  ...
  Size do_get_origin(Size nbkt, const rocket::refcounted_ptr<Variable> &var) noexcept
    {
      ...
      const auto seed = static_cast<Uint32>(reinterpret_cast<Uintptr>(var.get()) * 0xBA0DC66B);
...
void Variable_hashset::do_reserve(Size res_arg)
  {
    ...
    const auto nbkt = res_arg | this->m_size * 3 | 127;
    if(nbkt > std::numeric_limits<Size>::max() / sizeof(rocket::refcounted_ptr<Variable>) / 2) {
      rocket::throw_length_error("Variable_hashset::do_reserve(): A table of `%lld` variables is too large and cannot be allocated.",
                                 static_cast<long long>(nbkt));
    }
